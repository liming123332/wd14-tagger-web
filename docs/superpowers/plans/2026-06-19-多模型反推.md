...
> naive-ui `n-select` 的 `render-label` 是 **prop（render function `(option, isSelected) => VNodeChild`），不是插槽**（v2.44 `SelectSlots` 只声明 default/header/action/empty/arrow，无 renderLabel；运行时 NSelect 消费 `props.renderLabel`，见 `Select.d.ts:102`）。它同时作用于「下拉面板每一项」与「选中态 trigger 显示」，故无论面板是否展开，已选模型的状态标签都会渲染；未下载模型选中时下方的下载条 `v-if="!selectedDownloaded"` 额外提供「下载」入口。
...
把 vue import 补 `watch, h`（Step 4 已补 `computed, onMounted`）：
