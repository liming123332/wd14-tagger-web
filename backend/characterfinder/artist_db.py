...
# NOTE: 不在模块顶层把 paths.ARTISTS_DB 绑定到默认参数（_DEFAULT_DB = paths.X）。
# 顶层绑定会在 import 时固化 Path 对象，之后测试用 monkeypatch 改 paths.ARTISTS_DB
# 无法影响已绑定的默认值，导致跨测试状态泄漏（Task 8 全量回归红灯）。
# 改为 __init__ 内惰性读取 paths.ARTISTS_DB，保证 monkeypatch 随时生效。
...
class ArtistDB:
    def __init__(self, db_path: Optional[Path] = None):
        self._path = db_path if db_path is not None else paths.ARTISTS_DB
